...
module.exports = dir => {
  ...
  db.ensureIndex('triple-subject', (key, value, emit) => {
    if (value.type !== 'triple') return;
    if (value.subject === undefined) return;
    emit(value.subject);
  });

  db.ensureIndex('triple-predicate', (key, value, emit) => {
    if (value.type !== 'triple') return;
    if (value.predicate === undefined) return;
    emit(value.predicate);
  });

  db.ensureIndex('triple-object', (key, value, emit) => {
    if (value.type !== 'triple') return;
    if (value.object === undefined) return;
    emit(value.object);
  });
